...
import {
  ShieldCheck, User as UserIcon, LogOut, Menu,
  Home, ShoppingBag, MapPin, MoreHorizontal,
  FileText, Lock, Moon, Sun,
} from "lucide-react";
import { Sheet, SheetContent, SheetTrigger } from "@/components/ui/sheet";
import { Logo } from "./logo";
import { useTheme } from "./theme-provider";

/* ── Adaptive glass styles ── */
...
function SlidingIndicator() {
  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-0.5">
      <span
        className="pointer-events-none absolute inset-y-0 rounded-full transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]"
        style={{
      ...
      <header className="fixed top-0 inset-x-0 z-50 px-4 pt-3 pointer-events-none">
        ...
        <div className="flex w-full items-center lg:hidden pointer-events-auto gap-2">
          <div
            ...
            style={{ ...pill, paddingTop: "10px", paddingBottom: "10px", paddingLeft: "14px", paddingRight: "14px" }}
          >
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-[86vw] max-w-sm bg-background/95 backdrop-blur-xl">
              <div className="mt-8 flex flex-col gap-2">
                ...
                <div className="mt-2 border-t pt-2"><ThemeSheetRow /></div>
                {!loading && user && (
                  <button
                    onClick={signOut}
        ...
        <div className="hidden lg:flex w-full items-center justify-center relative pointer-events-auto">
          ...
          <div className="absolute right-0"><ThemeCircle /></div>
        </div>
      </header>
...
function BottomNav({
  ...
  user: any; isAdmin: boolean; loading: boolean; signOut: () => void;
}) {
  const path = useRouterState({ select: (s) => s.location.pathname });
  const navigate   = useNavigate();
    ...
    >
      ...
      >
        ...
        <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
          ...
          <SheetContent side="bottom" className="rounded-t-3xl px-0 pb-0 bg-white border-0 shadow-none">
            <div className="flex flex-col" style={{ paddingBottom: "max(24px, env(safe-area-inset-bottom))" }}>
              ...
              {!loading && user ? (
                <BottomMenuRow icon={UserIcon} label="Account" sublabel={user.email}
                  onPress={() => { navigate({ to: "/dashboard" }); setMenuOpen(false); }} />
              ) : !loading ? (
                <BottomMenuRow icon={UserIcon} label="Log in"
                  onPress={() => { navigate({ to: "/login" }); setMenuOpen(false); }} />
              ) : null}

              {!loading && user && isAdmin && (
                <BottomMenuRow icon={ShieldCheck} label="Admin panel"
                  onPress={() => { navigate({ to: "/admin/bundles" }); setMenuOpen(false); }} accent />
              )}

              <div className="mx-5 my-2 border-t border-gray-100" />
              <BottomMenuRow icon={Lock}     label="Privacy Policy"    onPress={() => { navigate({ to: "/privacy" }); setMenuOpen(false); }} />
              <BottomMenuRow icon={FileText} label="Terms of Service"  onPress={() => { navigate({ to: "/terms" });   setMenuOpen(false); }} />

              {!loading && user && (
                <>
                  <div className="mx-5 my-2 border-t border-gray-100" />
                  <BottomMenuRow icon={LogOut} label="Sign out"
                    onPress={() => { signOut(); setMenuOpen(false); }} danger />
                </>
              )}
            </div>
          </SheetContent>
...
function BottomMenuRow({ icon: Icon, label, sublabel, onPress, danger, accent }: {
  icon: React.ElementType; label: string; sublabel?: string;
  onPress: () => void; danger?: boolean; accent?: boolean;
}) {
  const color = danger ? "#ef4444" : accent ? "#0066ff" : "#111827";
  const bg    = danger ? "#fef2f2" : accent ? "rgba(0,102,255,0.08)" : "#f3f4f6";
  return (
